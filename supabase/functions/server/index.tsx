...
// ─────────────────────────────────────────────────────────────────────────────
// AI 요약 프록시 (/ai-summary)
// - 인증 필요 (Bearer 토큰)
// - 프론트에서 OpenAI를 직접 호출하지 않고 이 엔드포인트를 거친다.
// - 환경변수: OPENAI_API_KEY (Supabase Dashboard > Edge Functions > Secrets)
// - 모델:     OPENAI_MODEL (기본값: gpt-4o-mini)
// ─────────────────────────────────────────────────────────────────────────────
...
app.post('/ai-summary', async (c) => {
  try {
    ...
    if (!openaiKey) {
      ...
      return c.json(
        { error: '관리자 설정이 필요합니다. OpenAI API Key가 설정되지 않았습니다.' },
        500
      );
    }
    ...
    const { reportData } = body as { reportData?: Record<string, unknown> };

    if (!reportData) {
      return c.json({ error: 'reportData가 필요합니다.' }, 400);
    }

    // 4) 프롬프트 구성
    const systemPrompt =
      '당신은 학교 AI 토론 프로그램 운영 결과를 분석하는 교육 전문가입니다. ' +
      '교사가 학부모·학교 관리자에게 보고할 수 있도록 핵심 성과와 개선점을 ' +
      '한국어 3~5문장으로 간결하게 요약하세요.';

    const userPrompt =
      `다음은 AI 토론 프로그램 운영 데이터입니다:\n${JSON.stringify(reportData, null, 2)}\n\n` +
      '이 데이터를 바탕으로 운영 결과 요약을 작성해주세요.';
    ...
    if (!openaiRes.ok) {
      ...
      console.error('[ai-summary] OpenAI error:', openaiRes.status, errText);
      return c.json(
        { error: `AI 요약 생성에 실패했습니다. (${openaiRes.status})` },
        502
      );
    }
    ...
    return c.json({ error: 'AI 요약 중 오류가 발생했습니다.' }, 500);
  }
});
